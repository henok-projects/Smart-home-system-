USERNAMES=( "galSmsService" "galEmailService" "galResourcesService" "galUsersService" "galHomesService" "galTestService" )#"galCertAuthDb" "galContinuousDb" "galHomesDb" "galHomesDataWarehouseDb" "galUsersDb" "galResourcesDb" "galTestDb")
...
for user in "${USERNAMES[@]}"; do
   if ! echo "$EXISTING_USERS" | grep -q "^$user\s"
    then
